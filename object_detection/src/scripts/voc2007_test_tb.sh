...
root=$(pwd)
data_root=$(pwd)/data
ckpt_root=$(pwd)/tbcheckpoints

for SLURM_ARRAY_TASK_ID in {2..2..1}
    do 
        #SLURM_ARRAY_TASK_ID 
        cfg=$(sed -n "$SLURM_ARRAY_TASK_ID"p ${root}/scripts/txt_files/tb.txt)
        ...
        save_path_test=${ckpt_root}/test/${netType}/${metric}_fixf${fix_fingers}_xminpos${xmin_pos}_${temp_balance_lr}_optim${optim_type}
        ...
        echo ................Start...........
        CUDA_VISIBLE_DEVICES=7 python YOLOv8/runtest.py \
            --net-type ${netType} \
            --dataset ${dataset} \
